    ...
    <div className="min-h-screen bg-cream">
      ...
      <nav className="px-4 sm:px-12 py-5 border-b border-border flex justify-between items-center bg-cream">
        ...
        <div className="flex items-center gap-4 sm:gap-8">
          <span className="hidden sm:block text-xs uppercase tracking-widest text-hint">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-12 py-8 sm:py-16">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8 sm:mb-12">
          <h1 className="font-serif text-3xl sm:text-4xl text-ink">Tus contratos</h1>
          <Link href="/templates" className="inline-block bg-ink text-cream px-6 py-3 text-[11px] font-medium tracking-[0.1em] uppercase hover:bg-green transition-colors text-center">
            + Nuevo contrato
          </Link>
        </div>
        ...
        {!user?.email_verified && (
          <div className="bg-amber-100 border border-amber-300 dark:bg-amber-950 dark:border-amber-800 px-4 sm:px-6 py-4 mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            {reenvioEstado !== "enviado" && (
              <button
                ...
                className="text-xs font-medium uppercase tracking-widest px-4 py-2 bg-amber-700 dark:bg-amber-800 text-white hover:bg-amber-900 transition-colors disabled:opacity-50 whitespace-nowrap self-start sm:self-auto"
              >
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-8 sm:mb-12">
          {[
            ...
            <div key={s.label} className="bg-surface border border-border p-4 sm:p-6">
              <p className="text-[10px] sm:text-xs uppercase tracking-widest text-hint mb-2 sm:mb-3">{s.label}</p>
              <p className="font-serif text-3xl sm:text-4xl text-ink">{s.value}</p>
            </div>
          ))}
        </div>
        ...
        {/* Empty state */}
        {contracts.length === 0 && (
          <div className="bg-surface border border-border text-center px-6 py-12 sm:py-16">
            ...
            <div className="flex justify-center">
              <Link
                href="/templates"
                className="text-xs font-medium uppercase tracking-widest px-6 py-3 bg-ink text-cream hover:bg-green transition-colors inline-block"
              >
        ...
        {contracts.length > 0 && (
          <>
            {/* Desktop table */}
            <div className="hidden sm:block bg-surface border border-border overflow-hidden">
            ...
            <div className="sm:hidden flex flex-col gap-3">
              {contracts.map((c) => (
                <div
                  key={c.id}
                  onClick={() => router.push(`/contratos/${c.id}`)}
                  className="bg-surface border border-border px-4 py-4 cursor-pointer hover:border-ink transition-colors"
                >
                  ...
                  <div className="flex justify-between items-center">
                    <p className="text-xs text-muted capitalize">{c.template_id.replace(/-/g, " ")}</p>
                    <p className="text-xs text-hint">{formatFecha(c.created_at)}</p>
                  </div>
                </div>
              ))}
            </div>
