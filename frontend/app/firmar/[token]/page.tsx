...
export default function FirmarPage() {
  ...
  async function handleFirmar() {
    ...
    try {
      ...
      if (json.error) { setErrorMsg(json.error); setSigning(false); return; }
      setEstado("firmado");
    } catch {
  ...
  if (estado === "firmado") {
    return (
      <div className="min-h-screen bg-cream flex flex-col items-center justify-center gap-4">
        ...
        <p className="font-serif text-3xl text-ink">Contrato firmado correctamente</p>
        <p className="text-sm text-muted">Te enviamos una confirmación por email.</p>
        <Link href="/" className="text-sm text-green underline underline-offset-4 mt-4">
          Ir a pact.ar
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <div className="max-w-3xl mx-auto px-12 py-16">
        ...
        <h1 className="font-serif text-3xl text-ink mb-2">{data?.contrato_nombre}</h1>
        ...
        {/* Aviso legal */}
        <div className="border border-border p-5 mb-8 bg-green-light">
        ...
        {errorMsg && <p className="text-xs text-red-600 mb-4">{errorMsg}</p>}
